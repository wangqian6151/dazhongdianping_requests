...
class Model():
    ...
    基类, 用来显示类的信息
    """
    db = pymongo.MongoClient()['DianPingPet']
...
def pet_from_li(li):
    ...
    if e('.comment-list').text():
        f.service = float(e('.comment-list b').eq(0).text())
        f.environment = float(e('.comment-list b').eq(1).text())
        f.economical = float(e('.comment-list b').eq(2).text())
...
def main():
    ...
    random.shuffle(pettype)
    random.shuffle(area)

    print('pettype {}'.format(pettype))
